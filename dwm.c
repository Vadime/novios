...
void buttonpress(XEvent *e)
{
	...
	for (i = 0; i < LENGTH(buttons); i++)
		if (buttons[i].func && buttons[i].button == ev->button && CLEANMASK(buttons[i].mask) == CLEANMASK(ev->state))
			buttons[i].func(0 && buttons[i].arg.i == 0 ? &arg : &buttons[i].arg);
}
...
createmon(void)
{
	...
	m->mfact = 0.5; /* factor of master area size [0.05..0.95] */
	m->nmaster = 1; /* number of clients in master area */
	m->showbar = 1;
	m->lt[0] = &layouts[0];
...
void drawbar(Monitor *m)
{
	...
	{ /* status is only drawn on selected monitor */
		...
		tw = TEXTW(m->ltsymbol);
		drw_text(drw, m->ww - tw, 0, tw, bh, lrpad, m->ltsymbol, 0);
		blw = tw + lrpad / 2;

		drw_setscheme(drw, scheme[SchemeNorm]);

		tw += TEXTW(stext);
		drw_text(drw, m->ww - tw, 0, tw - blw, bh, lrpad, stext, 0);
		blw = tw + lrpad / 2;

		// tw += TEXTW(stext);
		// drw_text(drw, m->ww - tw, 0, tw - blw, bh, 0, stext, 0);
		// blw = tw;
	}
	...
	{
		selectedTag = m->tagset[m->seltags] & 1 << i;
		if (occ & 1 << i || selectedTag)
		{
			w = TEXTW(tags[i]);
			drw_setscheme(drw, scheme[selectedTag ? SchemeSel : SchemeNorm]);
...
{
	die("NoviWM: An other x-session is open");
	return -1;
}
...
int main(int argc, char *argv[])
{
	...
	for (int i = 0; i < LENGTH(colors); i++)
	{
		...
		if (!drw || !(ret = ecalloc(2, sizeof(XftColor))))
		{
			scheme[i] = NULL;
			continue;
		}
		...
		for (j = 0; j < 2; j++)
		{
			if (!drw || !&ret[j] || !colors[i][j])
				continue;

			if (!XftColorAllocName(drw->dpy, DefaultVisual(drw->dpy, drw->screen),
								   DefaultColormap(drw->dpy, drw->screen),
								   colors[i][j], &ret[j]))
				die("error, cannot allocate color '%s'", colors[i][j]);
		}
	...
	if (XQueryTree(dpy, root, &d1, &d2, &wins, &num))
	{
		for (i = 0; i < num; i++)
		{
			if (!XGetWindowAttributes(dpy, wins[i], &xWindowAttributes) || xWindowAttributes.override_redirect || XGetTransientForHint(dpy, wins[i], &d1))
				continue;
			if (xWindowAttributes.map_state == IsViewable || getstate(wins[i]) == IconicState)
				manage(wins[i], &xWindowAttributes);
		}
		for (i = 0; i < num; i++)
		{ /* now the transients */
			if (!XGetWindowAttributes(dpy, wins[i], &xWindowAttributes))
				continue;
			if (XGetTransientForHint(dpy, wins[i], &d1) && (xWindowAttributes.map_state == IsViewable || getstate(wins[i]) == IconicState))
				manage(wins[i], &xWindowAttributes);
		}
		if (wins)
			XFree(wins);
	...
	while (mons)
		cleanupmon(mons);
	for (k = 0; k < CurLast; k++)
		drw_cur_free(drw, cursor[k]);
	for (k = 0; k < LENGTH(colors); k++)
		free(scheme[k]);
